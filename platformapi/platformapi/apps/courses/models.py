...
class Course(BaseModel):
    ...
    def discount(self):
        ...
        last_activity_log = self.price_list.filter(activity__end_time__gt=now_time, activity__start_time__lt=now_time).order_by('-id').first()
    ...
    def can_free_study(self):
        # 是否允许试学
        lesson_list = self.lesson_list.filter(is_deleted=False, is_show=True).order_by("order").all()
...
class CourseLesson(BaseModel):
    ...
    lesson_link = models.CharField(max_length=255, blank=True, null=True, help_text="视频地址或者视频id, 文档地址", verbose_name="课时链接")
...
class Discount(BaseModel):
    ...
    sale = models.TextField(verbose_name="优惠公式", help_text="""0表示免费；<br>*号开头表示折扣价；<br>-号开头表示减免价；<br>""")
