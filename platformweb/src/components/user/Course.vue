...
const get_course_type_list = ()=>{
  course.get_course_type_list().then(res=>{
    course.course_type = res.data
  })
}
...
const get_course_list = ()=>{
  ...
  course.get_user_course_list(token).then(res=>{
    course.user_course_list = res.data.results // 我的课程列表
    course.user_course_count = res.data.count   // 我的课程总数
  })
}
get_course_list()
